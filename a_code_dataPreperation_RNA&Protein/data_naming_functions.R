...
name_data<-function(initialValue, # can be "genes0.05", "genes_P0.05Fold2"
                    ...
                    roundData, # True or False
                    sumTechnicalReplicates, # True or False
...
pick_data_n<-function(dataType,initialValue){
  ...
  {
    metaData_mrna=read.csv(paste0("../a_results/","metaRNA.csv"))
    metaData_protein=read.csv(paste0("../a_results/","metaProtein.csv"))
    
    metaData_mrna$dataSet=paste0(metaData_mrna$dataSet,"_0")
    
    proteinSampleNames=metaData_protein$dataSet
    replaceTo = grep(pattern = "_1$",x = proteinSampleNames,value = TRUE)
    replaceFrom = gsub(pattern = "_1$", replacement = "_0", x = grep(pattern = "_1$",x = proteinSampleNames,value = TRUE))
    
    metaData_mrna%>%
      dplyr::filter(dataSet %in% replaceFrom) %>%
      dplyr::mutate(dataSet=replace_fun(input_vector = dataSet, initialVal = replaceFrom, finalVal = replaceTo))->metaData_mrna_v2
    
    dplyr::bind_rows(metaData_mrna,metaData_mrna_v2)->metaData_mrna_duplicated
    
    dataSetVector=intersect(metaData_mrna_duplicated$dataSet, metaData_protein$dataSet)
    
    metaData_mrna_duplicated %>%
      dplyr::filter(dataSet %in% dataSetVector)->metaData
  }
...
test_data_function_n<-function(dataInput=mainData_internal, test_for)
{
  ...
  # control test_for
  if(!test_for %in% c("Mg_mM_Levels", "Na_mM_Levels", "growthPhase", "carbonSource", "noTest"))
    {stop("test_for should be in one of \"Mg_mM_Levels\", \"Na_mM_Levels\", \"growthPhase\", \"carbonSource\", \"noTest\"")}
