###*****************************
# @Title filter_data function NEW
# The data filtering function that controls sub functions.
filter_data<-function(dataType,
                      ...
                      normalizationMethodChoice) # can be "vst", "rlog", "log10", "noNorm") 
{
  ...
  if (!dataType %in% c("int_mrna_protein", "int_protein_mrna", "int_mrna", "int_protein"))
  {
  ...
  if (dataType %in% c("int_mrna_protein", "int_mrna", "int_protein"))
  {
    # Part1
    ###*****************************####
    mainData_internal=pick_data(dataType="mrna")
    mainData_internal=prepeare_data(dataInput = mainData_internal,
                                    roundData,
                                    sumTechnicalReplicates)
    ...
    for(counter01 in 1:length(mainData_internal$objectName))
    {mainData_internal$objectName[[counter01]]=as.character(mainData_internal$objectName[[counter01]])}
    mainData_internal_part1=mainData_internal
    ###*****************************####
    # End part 1
    ...
    # Part2 
    ###*****************************####
    mainData_internal=pick_data(dataType="protein")
    mainData_internal=prepeare_data(dataInput = mainData_internal,
                                    roundData,
                                    sumTechnicalReplicates)
    ...
    for(counter01 in 1:length(mainData_internal$objectName))
    {mainData_internal$objectName[[counter01]]=as.character(mainData_internal$objectName[[counter01]])}
    mainData_internal_part2=mainData_internal
    ###*****************************####
    # End part 2
    ...
    # Combine Results
    ###*****************************####
    deseq_DataObj1=mainData_internal_part1[[1]]
    deseq_DataObj2=mainData_internal_part2[[1]]
    
    objectName_1=mainData_internal_part1[[2]]
    objectName_2=mainData_internal_part2[[2]]
    
    res_df_1<-as.data.frame(assay(deseq_DataObj1))
    metaData_1<-as.data.frame(colData(deseq_DataObj1))
    
    res_df_2<-as.data.frame(assay(deseq_DataObj2))
    metaData_2<-as.data.frame(colData(deseq_DataObj2))
    
    intersectingSampleList=intersect(as.vector(metaData_1$dataSet), as.vector(metaData_2$dataSet))
    
    metaData_1 %>%
      dplyr::filter(dataSet %in% intersectingSampleList)%>%
      dplyr::rename(sizeFactor_mrna=sizeFactor)-> metaData_1b
    
    metaData_2 %>%
      dplyr::filter(dataSet %in% intersectingSampleList)%>%
      dplyr::rename(sizeFactor_protein=sizeFactor)-> metaData_2b
    dplyr::full_join(metaData_1b,metaData_2b) -> metaData
    
    res_df_1 %>%
      dplyr::select(one_of(intersectingSampleList)) %>%
      tibble::rownames_to_column(var = "gene_id")-> res_df_1_filtered
    res_df_2 %>%
      dplyr::select(one_of(intersectingSampleList)) %>%
      tibble::rownames_to_column(var = "gene_id")-> res_df_2_filtered
    
    # pick desired data type
    if(grepl("mrna",dataType)&grepl("protein",dataType))
    {dplyr::bind_rows(res_df_1_filtered,res_df_2_filtered)->res_df}
    if(grepl("mrna",dataType)&!grepl("protein",dataType))
    {res_df_1_filtered->res_df}
    if(!grepl("mrna",dataType)&grepl("protein",dataType))
    {res_df_2_filtered->res_df}
    
    rownames(res_df)<-as.vector(t(res_df["gene_id"]))
    res_df %>% dplyr::select(-gene_id)->res_df
    
    # select desired pick_data name
    if(grepl("mrna",dataType)&grepl("protein",dataType))
    {objectName=objectName_2;objectName$pick_data="int_mrna_protein"}
    if(grepl("mrna",dataType)&!grepl("protein",dataType))
    {objectName=objectName_2;objectName$pick_data="int_mrna"}
    if(!grepl("mrna",dataType)&grepl("protein",dataType))
    {objectName=objectName_2;objectName$pick_data="int_protein"}
    
    
    mainData_internal=list(objectName=objectName,metaData=metaData,res_df=res_df)
  }
