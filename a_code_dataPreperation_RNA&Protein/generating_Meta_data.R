...
setwd('/Users/umut/GitHub/ecoli_learning_bacterial_response_optimization/a_code_dataPreperation_RNA&Protein/') # mac computer
#***************************************
...
# # No need for this one MURI_83 is updated
...
# We still need this one MURI_82 for proteins is composed of NA
...
doubling_times %>% 
  dplyr::mutate(experiment=ifelse(name=="Gluconate.tab","gluconate_growth",NA)) %>%
  dplyr::mutate(experiment=ifelse(name=="Glucose.tab","glucose_time_course",experiment)) %>%
  dplyr::mutate(experiment=ifelse(name=="Glycerol.tab","glycerol_time_course",experiment)) %>%
  dplyr::mutate(experiment=ifelse(name=="Lactate.tab","lactate_growth",experiment)) %>%
  dplyr::mutate(experiment=ifelse(name=="MgSO4_000.080_mM.tab","MgSO4_stress_high",experiment)) %>%
  dplyr::mutate(experiment=ifelse(name=="MgSO4_000.800_mM.tab","MgSO4_stress_high",experiment)) %>%
  dplyr::mutate(experiment=ifelse(name=="MgSO4_008.000_mM.tab","MgSO4_stress_high",experiment)) %>%
  dplyr::mutate(experiment=ifelse(name=="MgSO4_050.000_mM.tab","MgSO4_stress_high",experiment)) %>%
  dplyr::mutate(experiment=ifelse(name=="MgSO4_200.000_mM.tab","MgSO4_stress_high",experiment)) %>%
  dplyr::mutate(experiment=ifelse(name=="MgSO4_400.000_mM.tab","MgSO4_stress_high",experiment)) %>%
  dplyr::mutate(experiment=ifelse(name=="MgSO4-2_000.005_mM.tab","MgSO4_stress_low",experiment)) %>%
  dplyr::mutate(experiment=ifelse(name=="MgSO4-2_000.010_mM.tab","MgSO4_stress_low",experiment)) %>%
  dplyr::mutate(experiment=ifelse(name=="MgSO4-2_000.020_mM.tab","MgSO4_stress_low",experiment)) %>%
  dplyr::mutate(experiment=ifelse(name=="MgSO4-2_000.040_mM.tab","MgSO4_stress_low",experiment)) %>%
  dplyr::mutate(experiment=ifelse(name=="MgSO4-2_000.080_mM.tab","MgSO4_stress_low",experiment)) %>%
  dplyr::mutate(experiment=ifelse(name=="NaCl_005_mM.tab","NaCl_stress",experiment)) %>%
  dplyr::mutate(experiment=ifelse(name=="NaCl_100_mM.tab","NaCl_stress",experiment)) %>%
  dplyr::mutate(experiment=ifelse(name=="NaCl_200_mM.tab","NaCl_stress",experiment)) %>%
  dplyr::mutate(experiment=ifelse(name=="NaCl_300_mM.tab","NaCl_stress",experiment)) -> doubling_times

'%!in%' <- function(x,y)!('%in%'(x,y))
doubling_times %>% 
  dplyr::mutate(Mg_mM=ifelse(experiment %!in% c("MgSO4_stress_low","MgSO4_stress_high"),0.8,NA)) %>%
  dplyr::mutate(Mg_mM=ifelse(experiment=="MgSO4_stress_high",gsub("_mM.tab","",gsub("MgSO4_","",.$name)),Mg_mM)) %>%
  dplyr::mutate(Mg_mM=ifelse(experiment=="MgSO4_stress_low",gsub("_mM.tab","",gsub("MgSO4-2_","",.$name)),Mg_mM)) %>%
  dplyr::mutate(Mg_mM=as.numeric(Mg_mM))%>% 
  dplyr::mutate(Na_mM=ifelse(experiment != "NaCl_stress",5,NA)) %>%
  dplyr::mutate(Na_mM=ifelse(experiment=="NaCl_stress",gsub("_mM.tab","",gsub("NaCl_","",.$name)),Na_mM)) %>%
  dplyr::mutate(Na_mM=as.numeric(Na_mM)) %>%
  dplyr::select(-name)->doubling_times
...
meta_data %>% dplyr::mutate(sampleNum=as.numeric(Sample.))%>% dplyr::select(-Sample.)->meta_data
...
meta_data %>% dplyr::mutate(experiment=ifelse(experiment=="Lactate_growth",
                                      "lactate_growth", as.character(experiment)))->meta_data
meta_data %>% dplyr::mutate(experiment=ifelse(experiment=="Gluconate_growth",
                                      "gluconate_growth", as.character(experiment)))->meta_data
meta_data %>% dplyr::mutate(experiment=ifelse(experiment=="MgSO4_stress" & batchNumber %in% c("022","023","024"),
                                      "MgSO4_stress_high", as.character(experiment)))->meta_data
meta_data %>% dplyr::mutate(experiment=ifelse(experiment=="MgSO4_stress" & batchNumber %in% c("025","026","027"),
                                      "MgSO4_stress_low", as.character(experiment)))->meta_data

# generate carbonSource Column
meta_data %>%
  dplyr::mutate(carbonSource=GrowthConditions)->meta_data
...
meta_data %>% 
  dplyr::mutate(Mg_mM=ifelse(grepl("[E,S][0-9]",.[["Batch"]]) & experiment %in% c("MgSO4_stress_high","MgSO4_stress_low"),
                              gsub("[0-9][0-9][0-9][E,S]","",as.character(.[["Batch"]])),
                              NA))%>%
  dplyr::mutate(Mg_mM=ifelse(is.na(Mg_mM),0.8,as.numeric(Mg_mM))) %>%
  dplyr::mutate(Mg_mM=ifelse(batchNumber %in% c("025","026","027"),Mg_mM/1000,Mg_mM))-> meta_data

# generate NaCl_mM
meta_data %>% 
  dplyr::mutate(Na_mM=ifelse(grepl("[E,S][0-9]",.[["Batch"]]) & experiment=="NaCl_stress",
                         gsub("[0-9][0-9][0-9][E,S]","",as.character(.[["Batch"]])),
                         NA)) %>%
  dplyr::mutate(Na_mM=ifelse(is.na(Na_mM),5,as.numeric(Na_mM))) -> meta_data


# generate concentration column
meta_data %>% 
  dplyr::group_by() %>% 
  dplyr::mutate(concentration=ifelse(grepl("[E,S][0-9]",.[["Batch"]]),
                              gsub("[0-9][0-9][0-9][E,S]","",as.character(.[["Batch"]])),
                              NA)) %>%
  dplyr::mutate(concentration=ifelse(is.na(concentration),NA,as.numeric(concentration)))-> meta_data
...
meta_data %>% 
  dplyr::mutate(growthPhase=ifelse(grepl("E",.[["Batch"]]),"exponential",NA)) %>%
  dplyr::mutate(growthPhase=ifelse(grepl("S",.[["Batch"]]),"stationary",growthPhase)) %>%
  dplyr::mutate(growthPhase=ifelse(grepl("[t,R]",.[["Batch"]]) & growthTime_hr<=14,
                            "exponential",growthPhase))%>%
  dplyr::mutate(growthPhase=ifelse(grepl("[t,R]",.[["Batch"]]) & growthTime_hr>14 & growthTime_hr<=48,
                            "stationary",growthPhase))%>%
  dplyr::mutate(growthPhase=ifelse(grepl("[t,R]",.[["Batch"]]) & growthTime_hr>48,
...
meta_data %>%
  dplyr::group_by(growthPhase,carbonSource,Mg_mM,Na_mM) %>%
  dplyr::summarize(replicaNum=length(growthPhase)) ->sampleSizeDf

sampleSizeDf%>%
  dplyr::group_by()%>%
  dplyr::mutate(uniqueCondition=sprintf("unique_condition_%02d",as.numeric(rownames(.)))) -> sampleSizeDf


sampleSizeDf%>%
  dplyr::select(-replicaNum)%>%
  dplyr::left_join(meta_data,.)->meta_data

# Generate Unique Condition 02 Column
meta_data %>%
  dplyr::group_by(growthPhase,carbonSource,Mg_mM_Levels,Na_mM_Levels) %>%
  dplyr::summarize(replicaNum=length(growthPhase)) ->sampleSizeDf02

sampleSizeDf02%>%
  dplyr::group_by()%>%
  dplyr::mutate(uniqueCondition02=sprintf("unique_condition_%02d",as.numeric(rownames(.)))) -> sampleSizeDf02
...
meta_data %>% dplyr::select(-tubesInStorage, -notesNGS)->meta_data




meta_data %>% 
  dplyr::select(-Batch,-GrowthConditions, -concentration) %>%
  dplyr::arrange(dataSet)->meta_data

meta_data %>% 
  dplyr::left_join(.,doubling_times)->meta_data
#********************************
