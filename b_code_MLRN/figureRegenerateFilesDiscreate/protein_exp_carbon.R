...
timeStampFile %>%
  ...
  dplyr::filter(growthPhase_names=="Exp") %>%
  dplyr::filter(numRepeatsFor_TestTrainSubset_Choice==60) %>%
  dplyr::filter(ndivision==25) %>%
  dplyr::filter(testConditions==paste0(c("carbonSource"),collapse = "_"))->chosenDataSetInfo
...
performanceDf %>%
  ...
  dplyr::mutate(experiment="protein_exp_carbon")->winnerModelsForEachExp
...
write.csv(x = winnerModelsForEachExp, file = "../b_results/model_performance_protein_exp_carbon.csv")
###*****************************
...
cowplot::save_plot(filename = "../b_figures/combinedParameters_protein_exp_carbon.jpeg", plot = combinedParameters, ncol = 3, nrow = 2)
###*****************************
...
#                      filename = paste0("../b_figures/fig_protein_exp_carbon.pdf"),
...
#                      filename = paste0("../b_figures/fig_withLegend_protein_exp_carbon.pdf"),
#                      base_aspect_ratio=1.333, base_height = 6, 
#                      units = "in", useDingbats=FALSE, limitsize=FALSE,
#                      ncol = 1.4, nrow=1.2)
#   
...
#        file = paste0("../b_figures/fig_obj_protein_exp_carbon.Rda"))
#   ###*****************************
# }
