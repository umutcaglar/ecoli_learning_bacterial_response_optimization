...
timeStampFile %>%
  dplyr::filter(pick_data=="int_mrna") %>%
...
performanceDf %>%
  ...
  dplyr::mutate(experiment="int_mRNA")->winnerModelsForEachExp
...
write.csv(x = winnerModelsForEachExp, file = "../b_results/model_performance_int_mRNA.csv")
###*****************************
...
cowplot::save_plot(filename = "../b_figures/combinedParameters_int_mrna.jpeg", plot = combinedParameters, ncol = 3, nrow = 2)
###*****************************
...
if(! exists("doNotSave"))
{
  ...
  cowplot::save_plot(figComb,
                     filename = paste0("../b_figures/fig_int_mRNA.pdf"),
  ...
  cowplot::save_plot(figComb_wL,
                     filename = paste0("../b_figures/fig_withLegend_int_mRNA.pdf"),
                     base_aspect_ratio=1.333, base_height = 6, 
                     units = "in", useDingbats=FALSE, limitsize=FALSE,
                     ncol = 1.4, nrow=1.2)
  ...
  save(list = c("figComb","figComb_wL","percentLegendObj","color_legend"),
       file = paste0("../b_figures/fig_obj_int_mRNA.Rda"))
