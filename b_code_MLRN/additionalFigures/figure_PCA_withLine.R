...
train_and_plot_svm <- function(train, kernel = "radial", type ="C", cost, gamma) {
  ...
  browser()
  p <- ggplot() + stat_contour(data = grid, aes(x = x, y = y, z = z), breaks = c(0),color="white") 
  p <- p + geom_point(data = train, aes(x, y, colour = as.factor(value)), alpha = 0.8, size=3)
  p <- p + scale_colour_manual(values = c("#8da0cb","#fc8d62"))
  ...
  p <- p + scale_x_continuous(limits=c(-50,100)) + scale_y_continuous(limits=c(-75,35)) + 
...
train_and_plot_svm <- function(train, kernel = "radial", type ="C", cost, gamma) {
  ...
  grid$value <- p
  grid$z <- decisionValues
  p <- ggplot() + stat_contour(data = grid, aes(x = x, y = y, z = z), breaks = c(0),color="blue") 
  p <- p + geom_point(data = train, aes(x, y, colour = as.factor(value)), alpha = 0.8, size=3)
  p <- p + scale_colour_manual(values = c("#8da0cb","#fc8d62"))
  p <- p + geom_text_repel(data = train, aes(x=x, y=y, label = dataSet), size = 3,fontface = 'plain',force = 2,point.padding = unit(0.5, 'lines'),colour="grey50", segment.color="grey")
  p <- p + scale_x_continuous(limits=c(-50,100)) + scale_y_continuous(limits=c(-75,35)) + 
...
fig02<-train_and_plot_svm(train=mapped_DF, kernel = "radial", type ="C", cost=20, gamma=1/2)
plot(fig02)
