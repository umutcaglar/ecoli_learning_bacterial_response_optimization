...
for(counter01 in 1:numRepeatsFor_TestTrainSubset_Choice) 
{
  ...
  classWeightDf%>%
    ...
    dplyr::mutate(weights=weights/sum(weights))->classWeightDfSub
  ...
  sum(as.vector(prediction_linear_DF)==as.vector(meta_df_Test$conditionInvestigated))
  
  probabilities = as.data.frame(attr(prediction_linear_DF, "probabilities"))
  probabilities %>%
    tibble::rownames_to_column(var = "dataSet")%>%
    tidyr::gather(key="prediction", value = "probability",-dataSet)->probabilities_tidy
  
  meta_df_Test%>%
    dplyr::select(dataSet, conditionInvestigated)%>%
    dplyr::left_join(probabilities_tidy,.) %>%
    dplyr::mutate(selectedCondition=ifelse(prediction==conditionInvestigated,
                                           "conditionInvestigated",
                                           "Not_conditionInvestigated"))->probabilities_tidy
  
  probabilities_tidy%>%
    dplyr::group_by(dataSet,selectedCondition)%>%
    dplyr::summarize(probability=max(probability))%>%
    dplyr::group_by(dataSet)%>%
    dplyr::mutate(probability=probability/sum(probability))-> probabilities_tidy_summary
  
  probabilities_tidy_summary %>%
    tidyr::spread(key = selectedCondition, value = probability)->probabilities_summary
  
  probabilities_summary%>%
    dplyr::mutate(conditionTested=ifelse(conditionInvestigated>Not_conditionInvestigated,
                                         "conditionInvestigated",
                                         "conditionInvestigated"))->conditionTested_DF
  
  conditionTested_DF %>%
    .$conditionTested %>% as.vector(.) -> conditionTested
  
  conditionTested<-factor(conditionTested,levels=c("conditionInvestigated","Not_conditionInvestigated"))
  
  rownames(probabilities_summary)<-NULL
  rownames(probabilities_summary)<-as.vector(probabilities_summary$dataSet)
  probabilities_summary$dataSet<-NULL
  
  linear.rocr <- ROCR::prediction(as.matrix(probabilities_summary)[,1], conditionTested)
  x.svm_linear.perf <- ROCR::performance(linear.rocr, "tpr","fpr")
  linear.rocr
  
  browser()
  plot(x.svm_linear.perf, col=6)
  
  
  
  
  
  for(counter02 in 1:nrow(classWeightDf))
  {
    chosenCondition=as.vector(classWeightDf$conditionInvestigated[counter02])
    
    probabilities_tidy %>%
      dplyr::mutate(selectedCondition=ifelse(chosenCondition==conditionInvestigated,
                                             "conditionInvestigated",
                                             "Not_conditionInvestigated"))->probabilities_sub_tidy
    ...
    probabilities_sub_tidy_summary %>%
      tidyr::spread(key = selectedCondition, value = probability)->probabilities_sub_summary
    
    rownames(probabilities_sub_summary)<-NULL
    rownames(probabilities_sub_summary)<-as.vector(probabilities_sub_summary$dataSet)
    probabilities_sub_summary$dataSet<-NULL
    
    meta_df_Test %>%
      dplyr::mutate(selectedCondition=ifelse(chosenCondition==conditionInvestigated,
                                             "conditionInvestigated",
                                             "Not_conditionInvestigated")) %>%
      dplyr::group_by()%>%
      dplyr::select(dataSet,selectedCondition)->meta_df_Test_sub
    
    meta_df_Test_sub%>%
      .$selectedCondition %>%as.vector(.)->conditionTested
    
    linear.rocr <- ROCR::prediction(as.matrix(probabilities_sub_summary)[,1], factor(conditionTested))
    x.svm_linear.perf <- ROCR::performance(linear.rocr, "tpr","fpr")
    
    plot(x.svm_linear.perf, col=6)
    
    browser()
    temp<-as.data.frame(x = t(c(distinctRuns=counter01,
                                model="linear", 
                                chosenCondition=chosenCondition)))
    temp$alpha.values=slot(x.svm_linear.perf,"alpha.values")
    temp$x.values=slot(x.svm_linear.perf,"x.values")
    temp$y.values=slot(x.svm_linear.perf,"y.values")
