# The machine learning script #Protein
...
# RF Tune parameters
ntree_list_RF=c(1000, 5000, 10000) # c(1000, 5000, 10000)
node_size_list_RF=c(1,2,3,4,5) # c(1,2,3,4,5)
mtry_list_RF=c(1,2,3,4,5,6,7) # c(1,2,3,4,5,6,7)

# combined set related variables
batch_correction_type="separate" # can be together or separate for joined datasets
...
metaRunParameters <- as.list(dataNameDF)
names(metaRunParameters) <- gsub(pattern = ".*\\.", replacement = "", x = names(metaRunParameters))

metaRunParameters$analyzeName <- analyzeName

metaRunParameters$numRepeatsFor_TestTrainSubset_Choice <- numRepeatsFor_TestTrainSubset_Choice
metaRunParameters$percentTest <- percentTest
metaRunParameters$percentTune <- percentTune

metaRunParameters$testConditions <- testConditions

metaRunParameters$dimReductionType <- dimReductionType
metaRunParameters$dimensionChoiceValue <- dimensionChoiceValue

metaRunParameters$batchCorrectionMethod <- batchCorrectionMethod
metaRunParameters$classWeightInputType <- classWeightInputType
metaRunParameters$similarDataClassifierForBatch <- similarDataClassifierForBatch

metaRunParameters$type_svmChoice <- type_svmChoice

metaRunParameters$crossValue <- crossValue
metaRunParameters$nrepeatValue <- nrepeatValue
metaRunParameters$samplingValue <- samplingValue

metaRunParameters$linear_range_cost_power <- linear_range_cost_power

metaRunParameters$radial_range_cost_power <- radial_range_cost_power
metaRunParameters$radial_range_gamma_power <- radial_range_gamma_power

metaRunParameters$sigmoid_range_cost_power <- sigmoid_range_cost_power
metaRunParameters$sigmoid_range_gamma_power <- sigmoid_range_gamma_power

metaRunParameters$ntree_list_RF <- mtry_list_RF
metaRunParameters$node_size_list_RF <- mtry_list_RF
metaRunParameters$mtry_list_RF <- mtry_list_RF

metaRunParameters$batch_correction_type <- batch_correction_type
metaRunParameters$tuning <- tuning
metaRunParameters$parallel_com <- parallel_com
metaRunParameters$costFunction <- costFunction
###*****************************####
...
{source("pipeline/machineLearning_mainCode_part1_carefulPick.R")}

if(dataNameDF$objectName.pick_data=="int_mrna_protein")
{source("pipeline/machineLearning_MainCodeComb_carefulPick.R")}
###*****************************
